...
public class DeliveryPersonController {
	...
	public ResponseEntity<TastyBytesResponse> signIn(@RequestBody Credentials cred) {
		...
		if(deliveryPersonDto == null)
			return TastyBytesResponse.error("Couldn't find Delivery Person with that credentials");
		return TastyBytesResponse.success(deliveryPersonDto);
	}
	
	@GetMapping("/deliverypersonhomepage/{id}")
	public ResponseEntity<TastyBytesResponse> findDeliveryPersonHomePageDetails(@PathVariable("id") int id){
		DeliveryPersonHomePageDto deliveryPersonDto = ordersService.getdeliveryPersonHomePageDtoById(id);
		if(deliveryPersonDto == null)
			return TastyBytesResponse.error("Couldn't find Delivery Person Details with that id");
		return TastyBytesResponse.success(deliveryPersonDto);
	}
	
	@PostMapping("/deliveryperson/{orderId}/{status}")
	public ResponseEntity<TastyBytesResponse> setStatusByOrder(@PathVariable("orderId") int orderId, @PathVariable("status") String status) {
		boolean updateStatus = ordersService.setStatusForOrder(orderId, status);
		if(!updateStatus)
			return TastyBytesResponse.error("Couldn't update status for order");
		return TastyBytesResponse.success("Order status updated");
	}
	
	@GetMapping("/orders/deliveryperson/{id}")
	public ResponseEntity<TastyBytesResponse> getAllOrdersbyCustomerId(@PathVariable("id") int deliveryPersonId) {
		...
		return TastyBytesResponse.success(dphpDtoList);
	}
	
	@PostMapping("/deliveryperson/arrivedorders/{deliverypersonId}")
	public ResponseEntity<TastyBytesResponse> getArrivedOrders(@PathVariable("deliverypersonId") int deliverypersonId){
		...
		if(orders == null || orders.isEmpty())
			return TastyBytesResponse.error("No orders assigned");
		
		//List<OrdersDto>ordersDtoList = DaoToEntityConverter.ordersToOrdersDto(orders);
		return TastyBytesResponse.success(orders);
		
	}
	
	@GetMapping("/deliveryperson/{deliverypersonId}/status/{status}")
	public ResponseEntity<TastyBytesResponse> getOrders(@PathVariable("deliverypersonId") int deliverypersonId, @PathVariable("status") String status){
	...
	public ResponseEntity<TastyBytesResponse> deliveryPersonSignUp(@RequestBody DeliveryPersonSignUpDto deliveryPersonSignUpDto) {
		boolean status = deliveryPersonService.addDeliveryPerson(deliveryPersonSignUpDto);
		if(status)
			return TastyBytesResponse.success("Delivery Person Added");
		return TastyBytesResponse.error("Delivery person could not be added");
	}
